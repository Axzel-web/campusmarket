...
export const LoadingAnimation = () => {
  const mustardRef = useRef<SVGSVGElement>(null);
  const meatRef = useRef<SVGSVGElement>(null);
  const sodaRef = useRef<SVGSVGElement>(null);
  const cartRef = useRef<SVGSVGElement>(null);

  useEffect(() => {
    if (!mustardRef.current || !meatRef.current || !sodaRef.current || !cartRef.current) return;

    const mustard = mustardRef.current;
    const meat = meatRef.current;
    const soda = sodaRef.current;
    const cart = cartRef.current;

    const path = [{ x: -250, y: 0 }, { x: -100, y: -90 }, { x: 0, y: 0 }];
    const path2 = [{ x: 250, y: 0 }, { x: 150, y: -80 }, { x: 60, y: 0 }];
    const path3 = [{ x: -170, y: 0 }, { x: -80, y: -70 }, { x: 70, y: 0 }];

    const tl = gsap.timeline({ repeat: -1 });
    tl.timeScale(1.8);

    // Initial states
    tl.set(mustard, { x: -250, y: 0, opacity: 0, scale: 2 })
      .set(meat, { x: 250, y: 0, opacity: 0, scale: 2 })
      .set(soda, { x: -170, y: 0, opacity: 0, scale: 2 })
      .set(cart, { x: -750 / 2 }); // Centered initial offset

    // Animation sequence to match the provided code
    tl.to(cart, { 
      x: 750 / 2, 
      duration: 2.1,
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-white flex items-center justify-center overflow-hidden">
      ...
      <div className="scene">
        {/* Products */}
        <svg viewBox="0 0 58 58" id="mustard" className="product" ref={mustardRef}>
          <g>
        ...
        <svg viewBox="0 0 49.818 49.818" id="meat" className="product" ref={meatRef}>
          <g>
            ...
            <path style={{ fill: '#C64940' }} d="M27.074,27.586c-5.37,0-7.605-3.694-7.633-3.74l1.727-1.01l-0.863,0.505l0.859-0.511 c0.108,0.179,2.714,4.335,9.738,2.105c1.54-0.794,12.038-6.002,15.619-2.289l-1.439,1.389c-1.979-2.052-9.229,0.576-13.332,2.714 l-0.154,0.064C29.892,27.364,28.389,27.586,27.074,27.586z" />
          </g>
        </svg>

        <svg viewBox="0 0 49 49" id="soda" className="product" ref={sodaRef}>
          <g>
            <path style={{ fill: '#E22F37' }} d="M9.5,27V5.918c0-1.362,0.829-2.587,2.094-3.093l0,0C12.642,2.406,13.5,1.14,13.5,0.011L13.5,0v0 l11,0l11,0v0v0.011c0,1.129,0.858,2.395,1.906,2.814l0,0c1.265,0.506,2.094,1.73,2.094,3.093V27v-5v21.082 c0,1.362-0.829,2.587-2.094,3.093h0c-1.048,0.419-1.906,1.686-1.906,2.814V49l0,0h-11h-11l0,0l0-0.011 c0-1.129-0.858-2.395-1.906-2.814h0c-1.265-0.506-2.094-1.73-2.094-3.093V22" />
        ...
        <div className="cart-container">
          <svg viewBox="0 0 512 512" id="cart" ref={cartRef}>
        ...
        <div className="brand-text">Campus Market</div>
      </div>
    </div>
  );
};
